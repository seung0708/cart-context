...
export default function Products() {
    ...
    return (
        <div className="flex flex-col justify-center bg-gray-100">
            <div className="flex justify-between items-cetner px-20 py-5">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-10">
                {
                    products.map(product => (
                        <div key={product.id} className="bg-white shadow-md rounded-lg px-10 py-10">
                            <img src={product.thumbnail} al={product.title} className="rounded-md h-48"/>
                            <div className="mt-4">
                                <h2 className="text-lg uppercase font-bold">{product.title}</h2>
                                <p className="mt-2 text-gray-600 text-sm">{product.description.slice(0, 50)}</p>
